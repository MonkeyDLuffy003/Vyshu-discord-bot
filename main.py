...
# Shared secret so ONLY your Flutter app can hit the control API below.
# Put the same value in the Flutter app's config — never commit it.
...
# ──────────────────────────────────────────────────────────────
# CONTROL API — lets the Vyshu AI Flutter app control this bot (NEW)
# ──────────────────────────────────────────────────────────────
# Endpoints (all require header:  X-API-Key: <API_SECRET_KEY>)
#   GET  /api/status                → bot online status + every server's mode
#   POST /api/mode  {guild_id,mode} → change a server's mode remotely
#   GET  /api/health                → simple uptime ping, no auth needed

def _check_api_key(request: web.Request) -> bool:
    if not API_SECRET_KEY:
        return False  # refuse everything if no key is configured — fail safe
...
async def handle_status(request: web.Request):
    ...
    cfg = load_server_config()
    guilds_info = [{"id": str(g.id), "name": g.name, "mode": get_server_mode(g.id)} for g in bot.guilds]
...
async def handle_set_mode(request: web.Request):
    if not _check_api_key(request):
        return web.json_response({"error": "unauthorized"}, status=401)
    try:
        ...
        return web.json_response({"error": "bad request, need {guild_id, mode}"}, status=400)
    if mode not in ("translate", "personal", "off"):
        return web.json_response({"error": "mode must be translate/personal/off"}, status=400)
    set_server_mode(guild_id, mode)
    return web.json_response({"ok": True, "guild_id": guild_id, "mode": mode})
...
async def start_api_server():
    ...
    await site.start()
    print(f"🌐 Control API listening on port {API_PORT}")
...
async def on_ready():
    ...
    if not hasattr(bot, "_api_started"):
        ...
        if API_SECRET_KEY:
            ...
            print("⚠️ API_SECRET_KEY not set — control API disabled (Flutter app can't connect)")
    try:
